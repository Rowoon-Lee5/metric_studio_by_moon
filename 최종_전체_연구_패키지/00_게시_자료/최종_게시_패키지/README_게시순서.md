...
`01_블로그용_Word`에는 각 글을 따로 올릴 수 있는 Word 파일 네 개와, 전체를 한 번에 읽을 수 있는 합본 한 개가 있다.
...
- ④: `08_book_excerpt_mixed_results.png` → `05_model_failure_coherence.png`

책 발췌 카드는 실제 2장 페이지 사진을 받으면 같은 위치에서 교체한다. 연구 그래프는 한국어로 제작된 최종 업로드용 이미지다.

## 원고 원본

`03_원고_Markdown`에는 Git으로 관리되는 원고 원본이 있다. 수치·근거·재현 링크를 수정해야 할 때는 Word가 아니라 이 원본에서 먼저 바꾼다.
